...
raw_hpp_dat_path <- function(now = lubridate::now()) {
  # read in file names from data-raw
  raw_data_files <-
  list.files(here::here("data-raw"), pattern = "review_91309_extracted.*csv")
 ...
 # suppressWarnings( # there will be duplicates
  file.path(here::here("data-raw", raw_data_files[most_recent])) #%>%
