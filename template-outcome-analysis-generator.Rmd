...
withr::with_dir(here::here(), {
  ...
  tar_load(drug_class_ma)
  tar_load(chronic_condition_ma)
})
...
```{r drug class}
drug_class_ma %>%
  filter(!is.na(intervention_class)) %>% 
  pivot_wider(
    id_cols = intervention_class,
    names_from = outcome, 
    values_from = studies,
    values_fill = 0
  ) %>% 
  ...
  tab_header(
    title = "Number of studies by class of drug",
    subtitle = "With at least 3 studies because meta-analysis is a technique for multiple studies"
  )

```

However, some of these are coming out as `NA` so need to investigate with Hollie. 
...
```{r chronic condition}
chronic_condition_ma %>% 
  filter(!is.na(chronic_condition)) %>% 
  pivot_wider(
    id_cols = chronic_condition,
    names_from = outcome, 
    values_from = studies,
    values_fill = 0
  ) %>% 
  ...
  tab_header(
    title = "Number of studies by type of chronic pain condition",
    subtitle = "With at least 3 studies because meta-analysis is a technique for multiple studies"
  ) 
...
```{r echo=TRUE}
# number of entries with chronic pain not recorded
...
Check chronic pain conditions with Hollie.
