...
list(
  ...
  tar_target(
    w_obs,
    w_obs_viable %>%
      left_join(w_study_par, by = c("study", "arm")) %>%
  ...
  tar_target(
    ...
    m_nma_summary_all %>%
      ...
      select(
        outcome, condition, intervention, ci_contains_0, everything()
      )
  ),
