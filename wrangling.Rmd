...
> ensure we have the same number of observations before and after study labelling
...
The first step in the pipeline is to take the list of dataset and convert them into a single long dataset. 
...
The first three columns are identification columns in each dataset.

```{r}
# eventually need to get here
obs_dim %>% pull(cols) %>% sum() - (3 * nrow(obs_dim))

# compare with long, this should be greater
tar_read(w_obs_long) %>% dim()

# how to count the number of non-nas?
