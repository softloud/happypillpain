...
```{r eval=FALSE}
# view the list of unmatched descriptions & how many matches
...
# view the unmatches that have more than one match
# mouse over to see the table 
# next chunk expands "unnests" those tables 
# creates new row for each match
# so, if three matches, three rows in next
tar_read(w_obs_scale_counts) %>% 
  filter(cat_n > 1) %>% 
  View()
